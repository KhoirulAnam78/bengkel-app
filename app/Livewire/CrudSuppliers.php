...
class CrudSuppliers extends Component
{
    ...
    protected $messages = [
        ...
        'name.unique' => 'Supplier sudah ada !',
    ];
    ...
    {
        $this->validate();
        DB::table('suppliers')->where('id', $this->proses_id)->update([
            'name' => $this->name,
            'no_telp' => $this->no_telp,
            'alamat' => $this->alamat,
        ]);
    ...
    {
        try {
            $data = DB::table('suppliers')->where('id', $this->proses_id)->update(['is_deleted', 1]);
    ...
    {
        return view('livewire.crud-suppliers');
    }
}
